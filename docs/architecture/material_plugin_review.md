...
The rest of the application is less general. The standard request and result types describe LC material, bias, director initialization, and theta convergence. The shared workflow package constructs LC bias fields and director solvers. The runner protocol retains static and time-dependent LC compatibility methods, but local execution now also accepts explicitly registered material operations composed from a run callable and a result-to-`RunData` adapter. Persistence still recognizes only LC workflow checkpoint types and stores theta-specific state. Product conversion combines a useful generic data model with LC-specific result adapters, while PR now owns its independent product adapter. The GUI has reusable workers, plotting views, workspace concepts, and beam/grid controls, but its experiment composition, physics panel, solver choices, run dispatch, continuation, and result handling remain LC-oriented.
...
| `src/lcprop/pr/` | Material-specific | PR material/solver specifications, state evolution, source intensity, optical-response conversion, workflow, geometry, and coupling validation belong to the PR model. This is also an appropriate material boundary. |
...
| `src/lcprop/persistence/` | Material-specific | The checkpoint implementations serialize LC static and time-dependent requests and theta state. The top-level dispatcher recognizes only those two workflow names and concrete checkpoint types. There is no material identifier or PR codec. |
